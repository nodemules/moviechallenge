angular.module('movieController', [])
	...
	.controller('mainController', ['$scope', '$http', 'Movies', function($scope, $http, Movies) {
		...
		$scope.createMovie = function() {
			...
			// validate the search1 to make sure that something is there
			// if form is empty, nothing will happen
			if ($scope.search1.text != undefined) {
				$scope.loading = true;

				// call the create function from our service (returns a promise object)
				Movies.create($scope.search1)

					// if successful creation, call our get function to get all the new movies
					.success(function(data) {
						$scope.loading = false;
						$scope.search1 = {}; // clear the form so our user is ready to enter another
